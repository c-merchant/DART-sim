program broadcaster
  ...
  do i = 0, N-1
     ...
     buffer(:) = rank  ! Initialize buffer with current process's rank
     call mpi_bcast(buffer, c, mpi_double_precision, root, mpi_comm_world, ierr)
     ! Check if the buffer received correctly
     if (rank /= root) then
        if (any(buffer /= root)) then  
                print *, 'Error in process ', rank, ': Buffer did not receive correctly from root ', root 
                call mpi_abort(mpi_comm_world, 1, ierr)
        endif
     endif
  end do
